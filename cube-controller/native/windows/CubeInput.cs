...
internal static class CubeInput
{
    ...
    {
        bool extended = vk == 0x5b || vk == 0x5c || (vk >= 0x21 && vk <= 0x2e) || (vk >= 0xad && vk <= 0xb3);
    ...
    private static int Main(string[] args)
    {
        ...
        if (args.Length == 1 && args[0] == "--self-test")
            return Marshal.SizeOf(typeof(INPUT)) == (IntPtr.Size == 8 ? 40 : 28) ? 0 : 3;
        if (args.Length < 1 || args.Length > 5) return 2;
        var keys = new List<ushort>();
        foreach (string arg in args) {
            ushort key;
            if (!ushort.TryParse(arg, out key) || key < 1 || key > 254) return 2;
            keys.Add(key);
        }
        var events = new List<INPUT>();
        foreach (ushort key in keys) events.Add(Key(key, false));
        for (int i = keys.Count - 1; i >= 0; i--) events.Add(Key(keys[i], true));
        ...
        if (sent != events.Count) {
            ...
            for (int i = keys.Count - 1; i >= 0; i--) release.Add(Key(keys[i], true));
